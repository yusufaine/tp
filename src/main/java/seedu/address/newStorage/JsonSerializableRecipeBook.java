...
class JsonSerializableRecipeBook {
     ...
     */
    @JsonCreator
    public JsonSerializableRecipeBook(@JsonProperty("persons") List<seedu.address.newStorage.JsonAdaptedRecipe> recipes) {
    ...
    public JsonSerializableRecipeBook(ReadOnlyRecipeBook source) {
        recipes.addAll(source.getRecipeList().stream().map(seedu.address.newStorage.JsonAdaptedRecipe::new).collect(Collectors.toList()));
    }
